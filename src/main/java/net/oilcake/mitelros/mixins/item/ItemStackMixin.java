...
public class ItemStackMixin {
   ...
   public List getTooltip(EntityPlayer par1EntityPlayer, boolean par2, Slot slot) {
      ...
      if (par2 && par1EntityPlayer.inCreativeMode() && !is_map) {
         ...
         if (ItemMap.isBeingExtended((ItemStack)ReflectHelper.dyCast(this))) {
            var5 = "Extended Map";
         } else {
      ...
      var4.addInformationBeforeEnchantments((ItemStack)ReflectHelper.dyCast(this), par1EntityPlayer, var3, par2, slot);
            ...
            for(experience_cost = 0; experience_cost < var14.tagCount(); ++experience_cost) {
               ...
               if (Enchantment.enchantmentsList[required_heat_level] != null) {
                  if (Enchantment.enchantmentsList[required_heat_level].isReverse()) {
                     var3.add(EnumChatFormatting.RED + Enchantment.enchantmentsList[required_heat_level].getTranslatedName(hypothetical_level, (ItemStack)ReflectHelper.dyCast(this)));
                  } else if (Enchantment.enchantmentsList[required_heat_level].isTreasure()) {
                     var3.add(EnumChatFormatting.getByChar('6') + Enchantment.enchantmentsList[required_heat_level].getTranslatedName(hypothetical_level, (ItemStack)ReflectHelper.dyCast(this)));
                  } else {
                     var3.add(EnumChatFormatting.AQUA + Enchantment.enchantmentsList[required_heat_level].getTranslatedName(hypothetical_level, (ItemStack)ReflectHelper.dyCast(this)));
                  }
               }
            }
         }
      ...
      var4.addInformation((ItemStack)ReflectHelper.dyCast(this), par1EntityPlayer, var3, par2, slot);
      ...
      if (par2 && this.getQuality() != null) {
         ...
         if (modifier < 1.0F) {
            var3.add(EnumChatFormatting.RED + Translator.getFormatted("item.tooltip.durabilityPenalty", new Object[]{(int)((1.0F - modifier) * 100.0F)}));
         } else if (modifier > 1.0F) {
            var3.add(EnumChatFormatting.BLUE + Translator.getFormatted("item.tooltip.durabilityBonus", new Object[]{(int)((modifier - 1.0F) * 100.0F)}));
         }
      }
      ...
      if (slot instanceof SlotCrafting) {
         ...
         if (experience_cost == 0 && slot_crafting.getNumCraftingResultsC(par1EntityPlayer) > 1) {
            ...
            if (level_cost == 0) {
               ...
               Translator.addToListFormatted(EnumChatFormatting.YELLOW, "container.crafting.qualityCostMoreThanOneLevel", var3, new Object[]{level_cost});
            }
         }
   ...
   public boolean isEnchantable() {
      if (this.getItem() == Item.book) {
         return true;
      } else if (!ItemPotion.isBottleOfWater(this.getItem().getItemStackForStatsIcon()) && !ItemAppleGold.isUnenchantedGoldenApple(this.getItem().getItemStackForStatsIcon()) && !ItemGoldenAppleLegend.isUnenchantedGoldenApple(this.getItem().getItemStackForStatsIcon())) {
