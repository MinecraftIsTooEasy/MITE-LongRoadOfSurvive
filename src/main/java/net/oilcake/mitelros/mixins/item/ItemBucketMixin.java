...
public class ItemBucketMixin extends ItemVessel {
   ...
   public float getChanceOfMeltingWhenFilledWithLava() {
      ...
      return material != Material.adamantium && material != Materials.tungsten ? (material == Material.gold ? 0.5F : 0.025F * (Material.mithril.getDurability() / material.getDurability())) : 0.0F;
   }

   @Overwrite
   public IBehaviorDispenseItem getDispenserBehavior() {
      return (IBehaviorDispenseItem)(this.isEmpty() ? new DispenseBehaviorEmptyBucketRedirect((ItemBucket)this.getEmptyVessel()) : (this.getContents() != Material.water && this.getContents() != Material.lava ? null : new DispenseBehaviorFilledBucketRedirect((ItemBucket)this.getEmptyVessel())));
   }
                        ...
                        if (material_in_bucket == Material.lava && world.getBlockMaterial(x, y, z) == Material.water) {
                           ...
                           if (player == null || !player.inCreativeMode()) {
                              if (material_in_bucket != Material.water && material_in_bucket != Materials.unsafe_water && material_in_bucket != Materials.dangerous_water) {
                                 if (material_in_bucket == Material.lava) {
                              ...
                              } else if (!allow_placement_of_source_block) {
