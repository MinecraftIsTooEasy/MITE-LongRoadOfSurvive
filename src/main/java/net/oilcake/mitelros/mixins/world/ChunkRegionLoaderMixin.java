...
public class ChunkRegionLoaderMixin {
   ...
   @Redirect(
      method = {"readChunkFromNBT(Lnet/minecraft/World;Lnet/minecraft/NBTTagCompound;)Lnet/minecraft/Chunk;"},
      at = @At(
   value = "INVOKE",
   target = "Lnet/minecraft/AnvilChunkLoader;handleSectionChecksumFailure(Lnet/minecraft/ExtendedBlockStorage;)V"
)
   )
   public void removeConflictFunction(AnvilChunkLoader _this, ExtendedBlockStorage var13) {
   }
   ...
   @Inject(
      ...
      method = {"getInvalidSectionBlockConversionIdsOrMetadata()[[I"},
      at = {@At(
   value = "INVOKE",
   target = "Lnet/minecraft/AnvilChunkLoader;same()[I",
   ordinal = 0,
   shift = Shift.AFTER
)}
   )
   private static void injectBlock(CallbackInfoReturnable cir, int[][] array) {
   }

   @Shadow
   @Final
   private  int calcEntityChecksum(Entity entity) {
      return 0;
   }

   @Shadow
   @Final
   private int calcSectionChecksum(int xz_and_seed_checksum_component, int section_y, byte[] bytes) {
      return 0;
   }

   @Shadow
   @Final
   private int calcTileEntityChecksum(TileEntity tile_entity) {
      return 0;
   }

   @Shadow
   @Final
   private int calcXZAndSeedChecksumComponent(Chunk chunk) {
      return 0;
   }
