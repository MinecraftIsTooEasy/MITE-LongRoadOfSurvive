...
public class DamageMixin {
   ...
   protected float applyTargetDefenseModifiers(EntityLivingBase target, EntityDamageResult result) {
      ...
      if (this.amount <= 0.0F) {
         ...
         if (this.getItemAttackedWith() != null && this.getItemAttackedWith().getItem() instanceof ItemMorningStar) {
         ...
         if (target instanceof EntityPlayer && effective_protection >= this.amount) {
            ...
            return Math.max(this.amount - effective_protection, (Boolean)StuckTagConfig.TagConfig.TagArmament.ConfigValue ? 0.0F : 1.0F);
         } else {
            return Math.max(this.amount - effective_protection, (Boolean)StuckTagConfig.TagConfig.TagInstinctSurvival.ConfigValue ? 0.0F : 1.0F);
         }
      }
   }
