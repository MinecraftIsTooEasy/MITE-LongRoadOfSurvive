...
public class WorldGenStoneCone extends WorldGenerator {
   ...
   public int getFlitteredBlockID(Random random) {
      if (random.nextInt(4) == 0) {
         ...
         return random.nextInt(4) == 0 ? Blocks.blockAzurite.blockID : Block.cobblestone.blockID;
      }
   }
               ...
               for(int j = -radius; j <= radius; ++j) {
                  if (this.CalRadius(radius, i, j) && par1World.getBlock(par3 + i, par4 - 3 + layer, par5 + j) != Block.bedrock) {
                     if (temp.nextInt(3) == 0 && radius >= 1) {
                        par1World.setBlock(par3 + i - 1, par4 - 3 + layer, par5 + j, this.getFlitteredBlockID(temp), 0, 2);
                        par1World.setBlock(par3 + i + 1, par4 - 3 + layer, par5 + j, this.getFlitteredBlockID(temp), 0, 2);
                        par1World.setBlock(par3 + i, par4 - 3 + layer, par5 + j - 1, this.getFlitteredBlockID(temp), 0, 2);
                        par1World.setBlock(par3 + i, par4 - 3 + layer, par5 + j + 1, this.getFlitteredBlockID(temp), 0, 2);
                     }

                     par1World.setBlock(par3 + i, par4 - 3 + layer, par5 + j, this.getFlitteredBlockID(temp), 0, 2);
                  }
               }
            }
