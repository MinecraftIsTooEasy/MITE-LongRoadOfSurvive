...
public class EntityLich extends EntityBoneLord implements IBossDisplayData {
   private EntityAIAvoidEntity aiAvoidPlayerStrategic = new EntityAIAvoidEntity(this, EntityPlayer.class, 6.0F, 1.1, 1.4);
   private EntityAIAvoidEntity aiAvoidPlayerPanic = new EntityAIAvoidEntity(this, EntityPlayer.class, 32.0F, 1.3, 1.5);
   ...
   protected void applyEntityAttributes() {
      ...
      this.setEntityAttribute(SharedMonsterAttributes.maxHealth, 75.0);
   }
      ...
      if (!this.getWorld().isRemote) {
         ...
         if (this.spawnCounter > 300 && !this.attack_mode) {
            if (this.getTarget() != null) {
               ...
               this.getTarget().attackEntityFrom(new Damage(DamageSource.divine_lightning, 5.0F));
            }

            EntityLichShadow guardian = new EntityLichShadow(this.worldObj);
            guardian.setPosition(this.posX + (double)this.rand.nextInt(8) - (double)this.rand.nextInt(8), this.posY, this.posZ - (double)this.rand.nextInt(8) + (double)this.rand.nextInt(8));
            guardian.refreshDespawnCounter(-9600);
            this.worldObj.spawnEntityInWorld(guardian);
            guardian.onSpawnWithEgg((EntityLivingData)null);
         ...
         if (this.getHeldItemStack() == null) {
            ...
            if (target != null && this.canSeeTarget(true)) {
               ...
               if (this.getHeldItemStack().itemID == Items.ShockWand.itemID) {
                  if (distance < 3.0 && this.getHealth() >= 20.0F) {
                     this.swapHeldItemStackWithStowed();
      ...
      if (can_evade && (this.num_evasions > 0 || this.getHealth() < 20.0F && this.rand.nextInt(8) != 0)) {
         ...
         if (this.tryTeleportAwayFrom(this.getTarget(), 6.0)) {
            if (this.getHealth() >= 20.0F) {
               ...
               shadow.onSpawnWithEgg((EntityLivingData)null);
   ...
   public void onDeath(DamageSource par1DamageSource) {
      ...
      List targets = this.getNearbyEntities(48.0F, 48.0F);
