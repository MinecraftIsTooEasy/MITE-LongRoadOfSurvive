...
public class BlockAnvilMixin extends BlockFalling{
    ...
    public int dropBlockAsEntityItem(BlockBreakInfo info) {
        Random random = new Random();
        if(statuses[0].equals("intact")){
            return this.dropBlockAsEntityItem(info, this.getIngotID(), 0, random.nextInt(20) + 10, 1F);
        }else if (statuses[1].equals("slightlyDamaged")){
            return this.dropBlockAsEntityItem(info, this.getIngotID(), 0, random.nextInt(15) + 5, 1F);
        }else if (statuses[2].equals("veryDamaged")){
            return this.dropBlockAsEntityItem(info, this.getIngotID(), 0, random.nextInt(10) + 1, 1F);
        }else{
            return 0;
        }
    }
